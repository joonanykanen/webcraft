...
async function play(record: WorldRecord, fresh: boolean): Promise<void> {
  ...
  const g = new Game({
    ...
    hooks: {
      ...
      onLookDegraded: () => {
        // The engine granted pointer lock and then let the cursor run, which turns `movementX` into
        // accelerated travel — the reported "mouse goes wild while a button is held". We are on the
        // cursor-hidden path now and will not ask again this session; say so once, and write the
        // preference down so the next session starts in the mode that works.
        notify(
          'Pointer lock is unreliable in this browser, so the mouse stays hidden instead of locking. ' +
            'Look is unaffected — Settings › Mouse capture can change it back.',
          'warn',
        );
        if (settings.lockMouse) applySettings({ ...settings, lockMouse: false });
      },
